...
void InverseTransformFromBitReverseRadix2WithStartingM(
    uint64_t* result, const uint64_t* operand, uint64_t n, uint64_t modulus,
    const uint64_t* inv_root_of_unity_powers,
    const uint64_t* precon_inv_root_of_unity_powers, uint64_t starting_m, uint64_t input_mod_factor,
    uint64_t output_mod_factor) {
  HEXL_CHECK(NTT::CheckArguments(n, modulus), "");
  HEXL_CHECK(inv_root_of_unity_powers != nullptr,
             "inv_root_of_unity_powers == nullptr");
  ...
  size_t t = 1;
  size_t root_index = 1;
  for (size_t m = n_div_2; m > starting_m; m >>=1){
    root_index += m;
    t <<= 1;
  }

  for (size_t m = starting_m; m > 1; m >>= 1) {
...
void InverseTransformFromBitReverseRadix2WithEndingM(
    uint64_t* result, const uint64_t* operand, uint64_t n, uint64_t modulus,
    const uint64_t* inv_root_of_unity_powers,
    const uint64_t* precon_inv_root_of_unity_powers, uint64_t ending_m, uint64_t input_mod_factor,
    uint64_t output_mod_factor) {
  HEXL_CHECK(NTT::CheckArguments(n, modulus), "");
  HEXL_CHECK(inv_root_of_unity_powers != nullptr,
             "inv_root_of_unity_powers == nullptr");
  ...
  for (size_t m = n_div_2; m > ending_m; m >>= 1) {
    size_t offset = 0;
